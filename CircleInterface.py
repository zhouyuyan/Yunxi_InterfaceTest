        ...
        if request_data_type == 'Form':
            ...
            try:
                if encryption == 'MD5':
                    ...
                    if status != 200:
                        L.e(num + ' ' + api_purpose + "[ " + str(status) + " ], 获取md5验证码失败！！！")
                        continue
                ...
                L.e(num + ' ' + api_purpose + ' 请求的数据有误，请检查[Request Data]字段是否是标准的json格式字符串！')
                continue
            ...
            if not os.path.exists(dataFile):
                L.e(num + ' ' + api_purpose + ' 文件路径配置无效，请检查[Request Data]字段配置的文件路径是否存在！！！')
                continue
...
------WebKitFormBoundaryDf9uRfwb8uzv1eNe--
    ''' % (os.path.basename(dataFile), data)
        status, resp = interfaceTest(num, api_purpose, api_host, request_url, request_data, check_point, request_method,
                                     request_data_type, correlationDict['${Cookie}'])
        if status != 200:
        ...
        for j in range(len(correlation)):
            ...
            if len(param) == 2:
                if param[1] == '' or not re.search(r'^\[', param[1]) or not re.search(r'\]$', param[1]):
                    L.e(num + ' ' + api_purpose + ' 关联参数设置有误，请检查[Correlation]字段参数格式是否正确！！！')
                    continue
                value = resp
...
def interfaceTest(num, api_purpose, api_host, request_url, request_data, check_point, request_method, request_data_type,
    ...
    login_data = {"mobile": "13071825896", "code": "3279"}
    login = requests.post('http://app.test.yunxi.tv/app/user/login', data=login_data, headers=headers)
    cookie = login.cookies
    url = 'http://' + api_host + request_url
    if request_method == 'POST':
        global rs
        rs = conn.post(url, request_data, headers=headers, cookies=cookie)



    elif request_method == 'GET':
        conn.get(url, headers=headers, cookies=cookie)
    else:
        L.e(num + ' ' + api_purpose + ' HTTP请求方法错误，请确认[Request Method]字段是否正确！！！')
        return 400, request_method
    status = rs.status_code
    resp = rs.text
    if status == 200:
        if re.search(check_point, str(resp)):
            L.i(num + ' ' + api_purpose + ' 成功, ' + str(status) + ', ' + str(resp))
            return status, json.loads(resp)
        else:
            L.e(num + ' ' + api_purpose + ' 失败！！, [ ' + str(status) + ' ], ' + str(resp))
            return 2001, resp
    else:
        L.e(num + ' ' + api_purpose + ' 失败！！！, [ ' + str(status) + ' ], ' + str(resp))
        return status, resp.encode(encoding='utf-8')
...
def sendreport():
    # 获取最新的报告
    report_dir = 'D:\\workspace\\TestInterfaceTest\\log\\'
